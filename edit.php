   ...
   require_once "pdo.php";
    ...
    if(!isset($_SESSION['username'])){
     ...
     if (isset($_POST['submit'])){
        ...
        if($_POST['cook']=='other_time'){
     		$cook=$_POST['text_cook'];
     	}else{
     		$cook=$_POST['cook'];
     	}
      ...
      if($_FILES['img']['tmp_name']){
        ...
        if (move_uploaded_file($_FILES['img']['tmp_name'], $target)) {
			...
			$query = "UPDATE `dishes` SET `name` = :name, `uploded_by` = :uploded_by, `prep`=:prep, `cook` = :cook, `ready_in`= :ready_in, `category` = :category, `serve` = :serve, `level` = :level, `discription` = :discription, `nutritions` = :nutritions WHERE `id`= :id";
...
<div class="field">
 ...
 <div class="field">
	 <label for="cook"><strong>Cooking Time:</strong></label><br>
  <select id="cook" name="cook" required>
        <option value="" style="display: none;"> Select Time</option>
        <option value="5 minutes" <?= ( $dish['cook'] == '5 minutes' )? 'selected': '' ?>>5 minutes</option>
        <option value="15 minutes" <?= ( $dish['cook'] == '15 minutes' )? 'selected': '' ?>>15 minutes</option>
        <option value="30 minutes" <?= ( $dish['cook'] == '30 minutes' )? 'selected': '' ?>>30 minutes</option>
        <option value="45 minutes" <?= ( $dish['cook'] == '45 minutes' )? 'selected': '' ?>>45 minutes</option>
        <option value="1 hour" <?= ( $dish['cook'] == '1 hour' )? 'selected': '' ?>>1 hour</option>
        <option value="1:30 hours" <?= ( $dish['cook'] == '1:30 hours' )? 'selected': '' ?>>1:30 hours</option>
        <option value="2 hours" <?= ( $dish['cook'] == '2 hours' )? 'selected': '' ?>>2 hours</option>
        <option value="other_time" <?= ($dish['cook'] == '5 minutes' || $dish['cook'] == '15 minutes' || $dish['cook'] == '30 minutes' || $dish['cook'] == '45 minutes' || $dish['cook'] == '1 hour' || $dish['cook'] == '1:30 hours' || $dish['cook'] == '2 hours')? '': 'selected'?> >Other Cooking Time</option>
  </select><br>
  <input type="text" id="text_cook" name="text_cook" placeholder="Other Cooking Time" value="<?= $dish['cook'] ?>"
  <?= ($dish['cook'] == '5 minutes' || $dish['cook'] == '15 minutes' || $dish['cook'] == '30 minutes' || $dish['cook'] == '45 minutes' || $dish['cook'] == '1 hour' || $dish['cook'] == '1:30 hours' || $dish['cook'] == '2 hours')? 'style="display: none;"': ''?>>
 </div>
...
<script>
  ...
  $('#cook').change(function(){
    if(cook.value === 'other_time'){
      document.querySelector('#text_cook').style.display = 'block';
      document.querySelector('#text_cook').required = true;
      document.querySelector('#text_cook').focus();
    }
    else{
      document.querySelector('#text_cook').style.display = 'none';
      document.querySelector('#text_cook').required = false;
    }
  });
